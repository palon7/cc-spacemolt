  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <span
          className={`text-xs uppercase tracking-wider ${isError ? 'text-red-400' : 'text-emerald-400'}`}
        >
          {isError ? 'Error' : 'Done'}
        </span>
      ...
      <div className="ml-3 flex flex-wrap gap-x-3 gap-y-0.5 text-sm">
      ...
      {entry.errors && entry.errors.length > 0 && (
        <div className="ml-3 mt-1 text-sm text-red-400">{entry.errors.join(', ')}</div>
      )}
    </div>
  );
}
